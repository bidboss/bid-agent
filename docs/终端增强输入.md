...
**职责**：readline 接管 + 候选列表渲染。仅依赖 `readline`（原生）、`ansi-escapes`、`chalk`、`./files/index.ts`。
...
在 `commandHandlers` 查表**之前**，主循环先尝试匹配 `.front/commands/<group>/<name>.md` 注册的指令：

```typescript
// 自定义指令分派：拆出指令名与 argsText，按 frontmatter.passthrough 决定行为
if (userInput.startsWith('/')) {
  const firstSpace = userInput.indexOf(' ');
  const cmdName = (firstSpace === -1 ? userInput : userInput.slice(0, firstSpace)).toLowerCase();
  const argsText = firstSpace === -1 ? '' : userInput.slice(firstSpace + 1).trim();

  const customResult = runCustomCommand(cmdName, argsText);
  if (customResult) {
    if (customResult.kind === 'print') {
      console.log(customResult.content);
      continue;
    }
    // passthrough：把模板正文作为附加段拼到 userInput 末尾，走 runOneTurn
    const composed = `${userInput}\n\n${customResult.content}`;
    await runOneTurn(composed);
    continue;
  }
}
```

`runCustomCommand` 在 `src/lc/commands/custom.ts` 实现，要点：

| 维度 | 行为 |
|---|---|
| 扫描范围 | `~/.front/commands/<group>/<name>.md`（用户级）+ `<cwd>/.front/commands/<group>/<name>.md`（项目级，项目级**覆盖**用户级同名） |
| 指令名 | `/<group>:<name>`（group 名禁用 `:` 与空白） |
| 文件格式 | YAML frontmatter（4 字段：`name` `description` `passthrough` `args_placeholder`），非法 frontmatter 降级为「首行 # 标题」兜底 |
| 缓存策略 | mtime 驱动而非 TTL，文件修改后下一次触发立即重读 |
| 占位符 | 正文内 `{{args}}` 在执行时替换为指令后追加的剩余文本；空时替换为空串 |
| 候选列表 | `getBuiltinCommands()` 合并内置 + 自定义，自定义指令自动出现在 `/` 候选中 |
| 帮助展示 | `/help` 末尾追加"自定义指令"分组 |
...
| 扩展点 | 实现起点 | 状态 |
|--------|----------|------|
| `@` 多选（一次引用多个文件） | `confirmSelection` 返回 `string[]` + 候选列表顶部多选指示 | 未支持 |
| 分页候选（超过 8 条） | `renderList` 增加 `listState.page` + `PageUp/PageDown` | 未支持 |
| 自定义指令（`.front/commands/`） | `src/lc/commands/custom.ts` + §6.3 | **已支持** |
| 命令面板视觉升级（图标 / 分组） | `renderList` 改造 | 未支持 |
